...
def _inject_implicit_mixed_number(step: str):
    r"""Automatically make a mixed number evalable e.g. 7 3/4 => 7+3/4."""
...
def _normalize(expr: str) -> str:
    r"""Normalize answer expressions."""
    if expr is None:
        return None
...
def last_boxed_only_string(string):
    r"""Find the last \\boxed{} or \\fbox{} command in the string."""
...
def remove_boxed(s):
    r"""Remove the \\boxed{} command from the string."""
    left = "\\boxed{"
    try:
...
def grade_answer_sympy_normalized(given_normalized: str, ground_truth_normalized: str) -> bool:
...
def grade_answer_mathd(given_answer: str, ground_truth: str) -> bool:
    r"""Grade the answer using mathd."""
...
def grade_answer_sympy(given_answer: str, ground_truth: str) -> bool:
    r"""Grade the answer using sympy."""
    try:
        given_sympy = latex2sympy(given_answer)
        ground_truth_sympy = latex2sympy(ground_truth)
        if sympy.simplify(given_sympy - ground_truth_sympy) == 0:
            return True
    except Exception as e:
        logger.debug(f"Failed to grade answer using sympy: {given_answer} and {ground_truth}")
        logger.debug(f"Error: {e}")

    try:
        given_normalized = _normalize(given_answer)
        ground_truth_normalized = _normalize(ground_truth)
        return grade_answer_sympy_normalized(given_normalized, ground_truth_normalized)
    except Exception as e:
        logger.debug(f"Failed to grade answer using sympy: {given_answer} and {ground_truth}")
        logger.debug(f"Error: {e}")
        return False
