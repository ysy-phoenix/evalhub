...
evalhub gen --model "$MODEL" --tasks humaneval --output-dir "$OUTPUT_DIR" --temperature "$TEMPERATURE" --max-tokens "$MAX_TOKENS"
evalplus.evaluate --dataset humaneval --samples "$OUTPUT_DIR/humaneval.jsonl"
evalhub gen --model "$MODEL" --tasks mbpp --output-dir "$OUTPUT_DIR" --temperature "$TEMPERATURE" --max-tokens "$MAX_TOKENS"
evalplus.evaluate --dataset mbpp --samples "$OUTPUT_DIR/mbpp.jsonl"

evalhub gen --model "$MODEL" --tasks livecodebench --output-dir "$OUTPUT_DIR" --temperature "$TEMPERATURE" --max-tokens "$MAX_TOKENS"
